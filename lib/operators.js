...
BinaryNode.prototype.dump = function() {
    return ['(', this.op, ' ', this.arg1.dump(), ' ', this.arg2.dump(), ')'].join('');
};
...
function chainR3(operator, parser) {
    function f(sepVal) {
        ...
        var val = new Arg(values[values.length - 1]);
        for (var i = ops.length - 1; i >= 0; i--) {
            val = new BinaryNode(ops[i], new Arg(values[i]), val);
        }
        return val;
    }
...
function chainL3(operator, parser) {
    function f(sepVal) {
        ...
        var val = new Arg(values[0]);
        for (var i = 0; i < ops.length; i++) {
            val = new BinaryNode(ops[i], val, new Arg(values[i+1]));
        }
        return val;
    }
    return C.app(f, C.sepBy1(parser, operator));
