...
function cut(message, parser) {
    ...
    return bind(getState, (state) => commit([[message, state]], parser));
}
...
function addError(e, parser) {
    ...
    function f(state) {
        return mapError(errors => F.cons([e, state], errors), parser);
    }
